...
Claude Code をどこで・どのように呼び出すか、何を渡してどんな出力を得るかを、実際に動くインフラとして示すことを目的としています。

デモのユースケースとして「CloudFormation スタックのデプロイ後に日本語レポートを自動生成する」シナリオを選択しています。このユースケースは、エージェントループが有効に機能する理由が分かりやすいという点で選んでいます：

- **複数の AWS API を順番に呼ぶ必要がある** — 単純な LLM 呼び出し一発では代替できない
- **ステップ数・順序がデプロイごとに変わりうる** — 静的なスクリプトでは対処しにくく、状況に応じた判断が必要
- **出力が構造化文書** — 仕様をプロンプトで表現しやすく、期待通りの形式が得られているか検証しやすい
- **誰でも「動いた」を確認できる** — レポートがブラウザで即座に見える
